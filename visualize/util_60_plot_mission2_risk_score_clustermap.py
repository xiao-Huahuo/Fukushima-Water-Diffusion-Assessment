...
def plot_risk_score_clustermap(input_paths, output_dir):
    ...
    try:
        ...
        # 为行（国家）创建颜色条
        row_colors = df_merged['风险等级'].map(risk_colors)

        # 绘制聚类热图
        g = sns.clustermap(df_norm, cmap='YlGnBu', fmt=".2f", linewidths=.5, linecolor='black',
                           row_colors=row_colors,
                           cbar_kws={'label': '标准化指标值'},
                           figsize=(10, 10))
        
        g.fig.suptitle('各国标准化风险指标聚类热图 (按风险等级着色)', fontsize=16, fontweight='bold', y=1.02)
        
        # 调整图例位置
        for label in risk_colors:
            g.ax_row_dendrogram.bar(0, 0, color=risk_colors[label], label=label, linewidth=0)
        l1 = g.ax_row_dendrogram.legend(loc="center right", bbox_to_anchor=(1.2, 0.8), title='风险等级', fontsize=10, title_fontsize=12)
        
        plt.tight_layout(rect=[0, 0, 1, 0.98]) # 调整布局以适应suptitle

        output_path = os.path.join(output_dir, "mission2_risk_score_clustermap.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制标准化风险指标聚类热图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_risk_score_clustermap(test_input_paths, test_output_dir)
