...
def plot_compliance_time_distribution_hist(input_paths, output_dir):
    ...
    try:
        ...
        sns.histplot(df_unique_schemes['达标时间t\''], kde=True, bins=len(df_unique_schemes), color='lightgreen', edgecolor='black')
        
        plt.title('处理方案最大达标时间分布直方图与KDE', fontsize=16, fontweight='bold')
        ...
        output_path = os.path.join(output_dir, "mission3_compliance_time_distribution_hist.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制最大达标时间分布直方图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_compliance_time_distribution_hist(test_input_paths, test_output_dir)
