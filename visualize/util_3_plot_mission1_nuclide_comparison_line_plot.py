...
def plot_nuclide_comparison_line_plot(input_paths, output_dir):
    ...
    sns.lineplot(x='时间', y='浓度', hue='核素', data=df_all, 
                 marker='o', markersize=4, linewidth=2, palette='viridis')

    plt.title(f'各核素在源点 ({SOURCE_LON}°E, {SOURCE_LAT}°N) 浓度随时间变化', fontsize=16, fontweight='bold')
    plt.xlabel('时间', fontsize=12)
    plt.ylabel('浓度 (Bq/m³)', fontsize=12)
    ...
    # 保存图片
    output_path = os.path.join(output_dir, "mission1_nuclide_comparison_line_plot.png")
...
if __name__ == "__main__":
    ...
    plot_nuclide_comparison_line_plot(test_input_paths, test_output_dir)
