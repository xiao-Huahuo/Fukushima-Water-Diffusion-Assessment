...
def plot_environmental_impact_boxplot_by_scheme(input_paths, output_dir):
    ...
    try:
        ...
        sns.boxplot(x='方案', y='环境影响', data=df_long, palette='viridis')
        
        plt.title('不同处理方案下环境影响分布箱线图', fontsize=16, fontweight='bold')
        ...
        output_path = os.path.join(output_dir, "mission3_environmental_impact_boxplot_by_scheme.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制环境影响箱线图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_environmental_impact_boxplot_by_scheme(test_input_paths, test_output_dir)
