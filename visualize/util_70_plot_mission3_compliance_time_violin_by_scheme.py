...
def plot_compliance_time_violin_by_scheme(input_paths, output_dir):
    ...
    try:
        ...
        sns.violinplot(x='方案_str', y='达标时间t\'', data=df_unique_schemes, inner="quartile", palette='mako')
        
        plt.title('不同处理方案下最大达标时间分布小提琴图', fontsize=16, fontweight='bold')
        plt.xlabel('处理方案', fontsize=12)
        plt.ylabel('最大达标时间 t\' (年)', fontsize=12)
        plt.xticks(fontsize=10)
        plt.yticks(fontsize=10)
        plt.yscale('log') # 达标时间可能跨越多个数量级
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        plt.tight_layout()

        output_path = os.path.join(output_dir, "mission3_compliance_time_violin_by_scheme.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制最大达标时间小提琴图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_compliance_time_violin_by_scheme(test_input_paths, test_output_dir)
