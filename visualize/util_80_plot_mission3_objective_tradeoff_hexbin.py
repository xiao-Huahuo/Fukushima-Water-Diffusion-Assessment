...
def plot_objective_tradeoff_hexbin(input_paths, output_dir):
    ...
    try:
        ...
        # 绘制hexbin图
        hb = plt.hexbin(x=df_unique_schemes['环境影响E30'], y=df_unique_schemes['成本C'], 
                        gridsize=10, cmap='Blues', bins='log', mincnt=1) # bins='log'使颜色更明显
        
        # 在hexbin图上叠加散点，并用达标时间着色
        scatter = sns.scatterplot(x='环境影响E30', y='成本C', hue='达标时间t\'', size='达标时间t\'', 
                                  sizes=(100, 1000), # 散点大小范围
                                  data=df_unique_schemes, palette='coolwarm', 
                                  legend='full', alpha=0.8, edgecolor='black', zorder=2)
        ...
        plt.title('处理方案目标权衡Hexbin图 (环境影响 vs. 成本, 达标时间着色)', fontsize=16, fontweight='bold')
        ...
        output_path = os.path.join(output_dir, "mission3_objective_tradeoff_hexbin.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制目标权衡Hexbin图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_objective_tradeoff_hexbin(test_input_paths, test_output_dir)
