...
def plot_cost_distribution_hist(input_paths, output_dir):
    ...
    try:
        ...
        sns.histplot(df_unique_schemes['成本C'], kde=True, bins=len(df_unique_schemes), color='lightcoral', edgecolor='black')
        
        plt.title('处理方案总成本分布直方图与KDE', fontsize=16, fontweight='bold')
        plt.xlabel('总成本 C', fontsize=12)
        plt.ylabel('方案数量', fontsize=12)
        plt.xticks(fontsize=10)
        plt.yticks(fontsize=10)
        plt.xscale('log') # 成本可能跨越多个数量级
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        plt.tight_layout()

        output_path = os.path.join(output_dir, "mission3_cost_distribution_hist.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制总成本分布直方图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_cost_distribution_hist(test_input_paths, test_output_dir)
