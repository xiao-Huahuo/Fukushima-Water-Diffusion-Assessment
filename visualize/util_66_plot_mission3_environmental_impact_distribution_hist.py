...
def plot_environmental_impact_distribution_hist(input_paths, output_dir):
    ...
    try:
        ...
        df = df.dropna(subset=['环境影响E30'])

        # 确保每个方案只有一个结果 (如果NSGA-II结果包含重复方案)
        df_unique_schemes = df.drop_duplicates(subset=['方案'])
        
        plt.figure(figsize=(10, 7))
        sns.histplot(df_unique_schemes['环境影响E30'], kde=True, bins=len(df_unique_schemes), color='lightsteelblue', edgecolor='black')
        
        plt.title('处理方案环境影响分布直方图与KDE', fontsize=16, fontweight='bold')
        plt.xlabel('环境影响 E30', fontsize=12)
        plt.ylabel('方案数量', fontsize=12)
        plt.xticks(fontsize=10)
        plt.yticks(fontsize=10)
        plt.xscale('log') # 环境影响可能跨越多个数量级
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        plt.tight_layout()

        output_path = os.path.join(output_dir, "mission3_environmental_impact_distribution_hist.png")
        plt.savefig(output_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"已生成: {output_path}")

    except Exception as e:
        print(f"绘制环境影响分布直方图时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    plot_environmental_impact_distribution_hist(test_input_paths, test_output_dir)
