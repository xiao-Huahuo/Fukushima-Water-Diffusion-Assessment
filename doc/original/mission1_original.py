...
# ============================================================================
class Config:
    INPUT_FILE = "raw_data/output/model_input_1deg.nc"
...
class OptimizedSolver:
    def __init__(self, ds, data_logger, status_logger):
        ...
        self.dx = 111320.0 * np.cos(np.radians(self.lat))
        self.dy = 111320.0
        self.dz = np.abs(np.diff(self.depth)).mean() if self.n_depth > 1 else 10.0
        ...
        self.status_log.info(f"网格: {self.n_depth}×{self.n_lat}×{self.n_lon}")
        self.status_log.info(f"源点: ({self.lon[self.source_i_lon]:.2f}°E, {self.lat[self.source_i_lat]:.2f}°N)")
    ...
    def add_source_term(self, dt):
        """添加源项"""
        dx_source = self.dx[self.source_i_lat]
        cell_volume = dx_source * self.dy * self.dz
...
def main(nuclide_name):
    ...
    nuclide_output_dir = os.path.join(Config.OUTPUT_DIR, nuclide_name)
    os.makedirs(nuclide_output_dir, exist_ok=True)

    Config.OUTPUT_NC = os.path.join(nuclide_output_dir, f"{nuclide_name}.nc")
    Config.DATA_LOG = os.path.join(nuclide_output_dir, f"data_log_{nuclide_name}.txt")
    Config.STATUS_LOG = os.path.join(nuclide_output_dir, f"status_log_{nuclide_name}.txt")
    Config.CHECKPOINT_DIR = os.path.join(nuclide_output_dir, f"checkpoints_{nuclide_name}")

    data_log, status_log = setup_logging()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python mission1_optimized.py <核素名>")
        print(f"可选: {list(Config.NUCLIDES_SIMULATION.keys())}")
    else:
        main(sys.argv[1])