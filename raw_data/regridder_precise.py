...
INPUT_DIR = "CMEMS/"
OUTPUT_DIR = "output/"
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "model_input_0p5deg.nc") # 修改输出文件名
...
# --- 2. 定义你的 0.5°x0.5° 目标网格 ---
# (网格中心点从 -59.5°S 到 64.5°N)
target_lats = np.arange(-59.5, 65, 0.5) # 修改步长为 0.5

# (网格中心点从 50.5°E 到 279.5°E)
# (280°E 即 80°W)
target_lons = np.arange(50.5, 280, 0.5) # 修改步长为 0.5
...
for var_key, file_path in FILES_TO_PROCESS.items():
    ...
    try:
        ...
        print("正在准备降采样 (Regridding) 到 0.5°x0.5° 网格...") # 更新打印信息
...
if len(all_regridded_datasets) != 5:
    ...
    final_dataset.attrs['description'] = 'Final 0.5x0.5 deg model input for diffusion simulation.' # 更新描述
    final_dataset.attrs['source'] = 'CMEMS (001_024) regridded to 0.5-deg' # 更新描述
